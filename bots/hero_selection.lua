...
-- Single matchup source: advantage table + IsSynergy/IsCounter helpers.
-- Generated by: npm run matchups (Stratz API)
...
local function ScoreCandidatesForTeam(team, rolePool, enemyNames, posIndex)
	...
	for _, cand in ipairs(rolePool) do
		if X.CanPickHero(team, cand) then
			...
			-- Uses matchups.IsSynergy/IsCounter (derived from advantage data, same file)
			if matchups.IsSynergy and #allyNames > 0 then
				for _, ally in ipairs(allyNames) do
					if ally ~= cand then
						if matchups.IsSynergy(cand, ally) then
							score = score + 1.5
						end
						if matchups.IsCounter and matchups.IsCounter(ally, cand) then
							score = score - 0.5
						end
					end
				end
...
local function InitPickScheduleOnce()
	...
	-- Debug:
	-- log("Pick schedule init @ "..string.format("%.2f", GameTime()))
